    ...
    public class RegisterCreatureCommandHandler : IRequestHandler<RegisterCreatureCommand>
    {
        ...
        public Task Handle(RegisterCreatureCommand command, CancellationToken cancellationToken)
        {
            var creature = new Creature(
                ...
                command.Speed.ToDictionary((KeyValuePair<string, int> x) => Enum.Parse<Speed>(x.Key), (KeyValuePair<string, int> x) => x.Value),
                command.Level,
                ...
                command.Senses.ToDictionary((KeyValuePair<string, int> x) => Enum.Parse<Senses>(x.Key), (KeyValuePair<string, int> x) => x.Value),
